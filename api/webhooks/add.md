...
subevents | Receive only specific status updates. Payload example: `{ "status_updates": ["sent", "failed"] }`. <br /><br /> Possible values by provider:<br />- Telnyx SMS: `queued`, `sending`, `sent`, `delivered`, `sending_failed`, `delivery_failed`<br />- Twilio SMS: `queued`, `sending`, `sent`, `delivered`, `undelivered`, `failed`<br />- Twilio WhatsApp: `queued`, `sending`, `sent`, `delivered`, `read`, `failed`, `undelivered`<br />- Gupshup WhatsApp: `queued`, `sent`, `delivered`, `read`, `failed`, `enqueued` | Array of string | No. Defaults to all status updates.
